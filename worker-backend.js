...
// لا حاجة لمكتبة الإكسيل هنا بعد الآن
...
app.post('/api/search', async (c) => {
  ...
  try {
    const results = await db.select().from(students).where(sql`name LIKE ${'%' + query + '%'} OR student_id LIKE ${'%' + query + '%'}`).all();
...
app.get('/api/managers', async (c) => {
    const db = drizzle(c.env.DB);
    try {
        const managers = await db.select({ id: users.id, username: users.username, role: users.role, created_at: users.created_at }).from(users).all();
...
app.post('/api/managers', async (c) => {
    ...
    try {
        ...
        return c.json({ success: true, message: 'Manager added' });
    } catch (e) {
        if (e.message.includes('UNIQUE')) return c.json({ error: 'Username exists' }, 409);
...
app.get('/api/stages', (c) => c.json([ { id: 'primary', name: "الابتدائية", /* ... */ } ]));
...
app.post('/api/admin/login', async (c) => {
  ...
  if (username === 'admin' && password === 'admin123') {
    const token = await new jose.SignJWT({}).setProtectedHeader({ alg: 'HS256' }).setExpirationTime('2h').sign(secret);
